...
Window {
   ...
   SocketContainerV2 {
      ...
      // Обновление координат при изменении activeSocketId
      Connections {
          target: control
          function onActiveSocketIdChanged(socketId) {
              if (socketId < 0) return
              
              // Используем двойную задержку, чтобы сокет успел раскрыться и layout обновился
              Qt.callLater(function() {
                  Qt.callLater(function() {
                      try {
                          var layout = socketsDummy.children[0]  // ColumnLayout
                          if (!layout) return
                          ...
                          if (!repeater || socketId >= repeater.count) return
                          
                          var socket = repeater.itemAt(socketId)
                          if (socket) {
                              var absolutePos = socket.mapToItem(null, 0, 0)
                              var newX = absolutePos.x
                              var newY = absolutePos.y
                              var newWidth = socket.width
                              var newHeight = socket.height
                              ...
                              if (control.activeSocketX !== newX || control.activeSocketY !== newY ||
                                  ...
                                  // Устанавливаем координаты через присваивание свойств
                          ...
                          console.log("Error updating socket coordinates:", error)
                      }
                  })
              })
          }
